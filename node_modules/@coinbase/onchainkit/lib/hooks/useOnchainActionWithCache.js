...
const inMemoryStorageService = require('../store/inMemoryStorageService.js');
...
function useOnchainActionWithCache(action, actionKey) {
  ...
  react.useEffect(() => {
    ...
    const callAction = async () => {
      ...
      if (actionKey) {
        fetchedData = await inMemoryStorageService.InMemoryStorage.getData(actionKey);
      }
      ...
      if (!fetchedData) {
        ...
        if (actionKey) {
          await inMemoryStorageService.InMemoryStorage.setData(actionKey, fetchedData);
        }
      }
      if (isSubscribed) {
        setData(fetchedData);
