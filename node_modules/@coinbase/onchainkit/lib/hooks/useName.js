...
const ensNameAction = address => async () => {
  try {
    return await client.publicClient.getEnsName({
      address
    });
  } catch (err) {
    return null;
...
const useName = address => {
  ...
  const _useOnchainActionWith = useOnchainActionWithCache.useOnchainActionWithCache(ensNameAction(address), ensActionKey),
    ensName = _useOnchainActionWith.data,
